...
def get_resolution_configs():
    """Get HLS resolution configurations."""
    return [
...
def build_ffmpeg_command(video_path, resolution, output_path, res_dir):
...
def convert_single_resolution(video_path, resolution, hls_dir):
...
def convert_video_to_hls(video_instance):
    ...
    try:
        for resolution in get_resolution_configs():
            convert_single_resolution(video_path, resolution, hls_dir)
